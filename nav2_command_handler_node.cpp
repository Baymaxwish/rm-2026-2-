...
public:
  NavCommandMapper() : Node("nav_command_mapper")
  {
    ...
    nav_command_subscriber_ = this->create_subscription<std_msgs::msg::Int8>(
      "/nav2_command", 10,   //改为sensorQOS通信
...
private:
  // 修改回调函数参数类型为Int8
  void nav_command_callback(const std_msgs::msg::Int8::SharedPtr msg)
  {
    RCLCPP_INFO(this->get_logger(), "收到命令: %d", msg->data);
    ...
    if (msg->data == 0) {
      ...
      goal_pose.pose.position.x = 0.3626737594604492;
      goal_pose.pose.position.y = -0.5779695510864258;
      goal_pose.pose.position.z = 0.0012054443359375;
  ...
  // 使用正确的Int8类型定义订阅者
  rclcpp::Subscription<std_msgs::msg::Int8>::SharedPtr nav_command_subscriber_;
